    ...
    def lookup_registration(self, name_first, name_last, dob, zipcode):
        try:
            ...
            if request:
                ...
                if len(sosrecs) > 1:
                    logger.debug("Found more than one match from SOS VV")
                    sosrecs = list(
                        filter(
                            lambda rec: zip_code_matches(rec["tree"], zipcode), sosrecs
                        )
                    )
                    logger.debug("filtered by ZIP to {} matches".format(len(sosrecs)))
