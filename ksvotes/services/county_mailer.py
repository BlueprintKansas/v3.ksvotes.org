...
class CountyMailer:
    def __init__(self, registrant, clerk, form_img_type):
        ...
        self.clerk_body = lazy_gettext("county_clerk_email")
    ...
    def set_clerk_subject(self, form_img_type):
        ...
        if form_img_type == "ab_forms":
            subject = "New Advance Ballot application(s) for {}".format(
    ...
    def get_body(self, form_img_type):
        body = "No Body"
        if form_img_type == "ab_forms":
            body = lazy_gettext("9AB_confirm_email")
        elif form_img_type == "vr_form":
            body = lazy_gettext("9_confirm_email")
        else:
            raise Exception("Unknown form_img_type %s" % (form_img_type))
        return body

    def set_receipt_subject(self, form_img_type):
        self.receipt_subject = self.subject_prefix() + lazy_gettext(
    ...
    def send(self):
        ...
        if not clerk_email or not reg_email:
            logger.error(
                "Missing clerk_email or reg_email for %s" % (self.registrant.session_id)
            )
            return {}

        attachments = self.build_attachments()

        logger.info("%s SEND mail to %s" % (self.registrant.session_id, clerk_email))
        responses = {}
        ...
        responses["clerk"] = self.ses.send_msg(msg, settings.EMAIL_FROM)
        logger.info(
            "%s SENT to clerk %s" % (self.registrant.session_id, responses["clerk"])
        )
        ...
        responses["receipt"] = self.ses.send_msg(receipt, settings.EMAIL_FROM)
        logger.info(
            "%s SENT receipt %s" % (self.registrant.session_id, responses["receipt"])
        )
    ...
    def build_attachments(self):
        ...
        for img_str in self.form_imgs:
            ...
            shasum = m.hexdigest()
            img_bin = base64.b64decode(
                img_str.replace("data:image/png;base64,", "")
                .replace('"', "")
                .replace("'", "")
            )
            att = {
                "name": str(self.registrant.session_id) + "-" + shasum + ".png",
                "img": img_bin,
            }
