...
class FormStep0(Form):
    ...
    dob = DOBField(
        ...
        validators=[
            ...
            Regexp(
                r"^\d{2}[\/\-]?\d{2}[\/\-]?\d{4}$", message=lazy_gettext("0_dob_flag")
            ),
        ],
    )
    ...
    email = EmailField(
        lazy_gettext("0_email"),
        validators=[
            DataRequired(message=lazy_gettext("Required")),
            Email(message=lazy_gettext("0_email_flag")),
        ],
    )
    phone = TelField(
        lazy_gettext("0_phone"),
        validators=[
            Optional(),
            Regexp(
                r"^\d{3}[\-\.]?\d{3}[\-\.]?\d{4}$", message=lazy_gettext("0_phone_help")
            ),
        ],
    )
