...
class FormVR3(FlaskForm):
    ...
    prev_addr = StringField(
        lazy_gettext("3_prev_addr"),
        validators=[RequiredIfBool("has_prev_addr", message=lazy_gettext("Required"))],
    )
    prev_unit = StringField(lazy_gettext("3_prev_unit"))
    prev_city = StringField(
        lazy_gettext("3_prev_city"),
        validators=[RequiredIfBool("has_prev_addr", message=lazy_gettext("Required"))],
    )
    prev_state = StringField(
        lazy_gettext("3_prev_state"),
        validators=[RequiredIfBool("has_prev_addr", message=lazy_gettext("Required"))],
    )
    prev_zip = StringField(
        lazy_gettext("3_prev_zip"),
        validators=[
            Optional(),
            RequiredIfBool("has_prev_addr", message=lazy_gettext("Required")),
            Regexp(r"^\d{5}(-\d{4})?$", message=lazy_gettext("3_zip_help")),
        ],
    )
