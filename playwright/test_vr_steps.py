...
def test_citizenship(page):
    ...
    click_submit(page)
    assert (
        page.locator("text=Voter registration is only available to U.S. citizens.")
        .inner_text()
        .strip()
        == "Voter registration is only available to U.S. citizens."
    )
...
def test_address(page):
    ...
    assert (
        page.locator("text=What is the address where you get your mail?").is_visible()
        == False
    )
...
def test_address_verification(page):
    ...
    assert (
        len(
            page.locator(
                "text=We couldn't verify your address with the Postal Service"
            ).all_text_contents()
        )
        == 1
    )
...
def test_preview(page):
    ...
    click_sign(page)
    if len(page.locator("text=Signature required").all_text_contents()) != 1:
        print(page.content())
